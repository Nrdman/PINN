...
def conv_continuity_constraint(laplacian, output, sobel_filter, use_tb=True):
    """
    div(sigma) = -f
    ...
    #Evan: insert -f = 2 pi ^2 sin (pi x) cos(pi y) into for loss

    if use_tb:
        return ((sigma1_x1 + sigma2_x2 +laplacian) ** 2).mean()
    else:
        return ((sigma1_x1 + sigma2_x2+laplacian) ** 2)[:, :, 1:-1, :].mean()

def conv_boundary_condition(output):
    left_bound, right_bound = output[:, 0, :, 0], output[:, 0, :, -1]
    top_down_flux = output[:, 2, [0, -1], :]
    loss_dirichlet = F.mse_loss(left_bound, torch.zeros_like(left_bound)) \
