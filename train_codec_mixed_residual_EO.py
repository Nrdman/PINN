...
from models.darcy_EO import conv_constitutive_constraint as constitutive_constraint
from models.darcy_EO import conv_continuity_constraint as continuity_constraint
from models.darcy_EO import conv_boundary_condition as boundary_condition
...
class Parser(argparse.ArgumentParser):
    def __init__(self): 
        ...
        self.add_argument('--epochs', type=int, default=20, help='number of epochs to train' ) #was 300
...
def laplacian(x,y):
    import math
    f_lap = np.zeros([64, 64])
    for i in range(64):
        for j in range(64):
            f_lap[i,j]  = 2*math.pi**2 *np.sin(x[i])*np.cos(y[j])
    f_lap = torch.tensor(f_lap,  dtype=torch.float64)
    return f_lap
...
if __name__ == '__main__':
    ...
    logger['nrmse_test'] = []
    import numpy as np
    xlist=np.linspace(1,64,64)/64
    f_lap = laplacian(xlist, xlist)
    def test(epoch):
        model.eval()
        loss_test = 0.
        relative_l2, err2 = [], []
        for batch_idx, (input, target) in enumerate(test_loader):
            ...
            loss_pde = constitutive_constraint(input, output, sobel_filter) \
                + continuity_constraint(f_lap, output, sobel_filter)
    ...
    with open(args.run_dir + "/args.txt", 'w') as args_file:
        json.dump(vars(args), args_file, indent=4)
    def test_funct():
        x=np.linspace(1,64,64)/64
        output = np.zeros([64, 64])
        for i in range(64):
            for j in range(64):
                output[i,j]  = np.sin(x[i])*np.cos(x[j])
        output = torch.tensor(output,  dtype=torch.double)
        return output
    def test_mesh():
        output = np.zeros([1,1,64,64])
        for i in range(64):
            for j in range(64):
                output[0,0, i, j]= 1
    ...
    print(test_funct())
    model.eval()
    model_test = model(test_mesh())
    l2_error= torch.linalg.norm(model_test-test_funct())
    l2_rel_error = l2_error/torch.linalg.norm(test_funct())
    print(f"L 2 absolute Error is:{l2_error}")
    print(f"L 2 relative Error is:{l2_rel_error}")
